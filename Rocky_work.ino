...
void BalanceRocky()
{
  ...
  // *** enter equations for input signals for v_c (left and right) in terms of the variables available ****
    v_c_R = v_d - measured_speedL*(Jp) - distLeft_m*(Ji) - dist_accum*(Js);
    //v_c_L = v_d - measured_speedR*(Jp) - distLeft_m*(Ji) - dist_accum*(Js);
    v_c_L = v_d- measured_speedR*(Jp) - distRight_m*(Ji) - dist_accum*(Js);
